...
- 单次执行：任务触发后仅运行一次  
- 周期执行：  
    - 调度周期：1/5/10/30 分钟、1/2/4/8 小时、1 天（默认 5 分钟）  
    - 执行规则：  
           - 短周期（<1 天）：整点触发（如 30 分钟周期在 00/30 分执行）  
           - 长周期（≥1 天）：需手动设置下次执行时间  
...
**处理流程配置**  
   ...
   | 模块          | 功能说明                      |
   |---------------|---------------------------|
   | 文本分段      | • 字符长度范围：100-2000（默认 800）<br>• 分段重叠：不超过设置的字段分段长度<br>• 预处理：标准化空白字符、移除 URL/邮箱等元数据        |
   | 图片描述      | 基于 Qwen/Qwen2-VL-72B-Instruct 模型生成图片内容描述                                         |
   | OCR 识别       | 采用 ucaslcl/GOT-OCR2_0 模型提取图像文字，支持精度调优                                       |
   | 自定义脚本    | 支持 Python 脚本扩展处理逻辑，可访问上下游数据                                               |
   | 文本嵌入      | 通过 BAAl/bge-m3 模型生成文本向量，输出兼容主流 NLP 框架                                     |
