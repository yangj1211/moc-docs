...
支持多种处理算子，包括：文本解析节点、图片解析节点、音频解析节点、视频解析节点、数据清洗节点和数据增强节点。

!!! note
    当前每个工作流最多只能包含一个数据增强节点，且该节点必须位于结束节点之前。换言之，数据增强要么对整个工作流中的数据生效，要么完全不生效，暂不支持对部分数据进行增强处理。
...
- 数据增强节点
   ...
   |---------------|---------------------------|
   | 节点名称       | 100 字符以内，工作流内唯一。                               |
   | 生成样本数      | 指定每个分段生成的样本数量。默认每个分段生成 200 个样本                            |
   | 数据格式        | 单轮指令 - 响应对）数据集的格式。包含 instruction（任务描述）、input（可选输入）和 output(预期输出），适用于训练对话、问答和任务型 Al 模型。示例：{"instruction"："翻译成法语"”input"'："Hello" "output": "Bonjour"'}                                        |
